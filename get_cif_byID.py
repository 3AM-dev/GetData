...
path = r'D:\机器学习--孙老师\data\data\Ti+Nb/'
x=[]
temp=[]
print("开始注册！")
with MPRester("K5JtfUOhunkvFWgT") as m:
    ...
    data = m.query(criteria={"elements": {"$all": ["Ti", "Nb"]}},#, "nelements": 2
                   properties=["cif", "e_above_hull", "band_gap", "density", "formation_energy_per_atom", "pretty_formula", "material_id"])
    # 通过参数设置得到不同的需要
...
# import numpy as np
# from pymatgen.ext.matproj import MPRester
# path = r'D:\机器学习--孙老师\data\my_try\Ti+Zr-OC/'
# x=[]
# temp=[]
# with MPRester("VNZxHJh7rSbOKaMl") as m:
#     print('注册成功，开始执行！')
#
#     data = m.query(criteria={"elements": {"$all": ["Ti", "Zr"], "$nin": ["O", "C"]}},
...
#
#
# with open('D:/机器学习--孙老师/data/my_try/Ti+Zr-OC/Ti+Zr-OC.csv', 'w', newline='') as csvfile:
#     writer = csv.writer(csvfile)
#     for row in x:
#         writer.writerow(row)
